...
def relative_mse_loss(
    prediction: Tensor, target: Tensor, mask_zero: bool = False
) -> float:
    """
    Compute MSE loss.
    ...
    target_mse = functional.mse_loss(
        masked_target, torch.ones_like(masked_target) * torch.mean(masked_target)
    )
...
def silog_loss(
    prediction: Tensor, target: Tensor, variance_focus: float = 0.85
) -> float:
    """
