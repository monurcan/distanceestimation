...
class DistNet_v16(pl.LightningModule):
    def __init__(self, pretrained=True, learning_rate=1e-3):
        super(DistNet_v16, self).__init__()
        ...
        self.fc_raytracing = nn.Sequential(
            nn.Linear(
                self.backbone_output_size + self.bbox_input_size,
                512,
            ),
            nn.GELU(),
            nn.Dropout(0.2),
            ...
            nn.Linear(
                128, 6
            ),  # small roll, small pitch, small heave, x_mid deviation, y_mid deviation, confidence
            nn.Tanh(),
        )
    ...
    def forward(self, roi, bounding_box_with_class):
        ...
        print("predicted_camera_height_above_water", predicted_camera_height_above_water[0])
        print("predicted_plane_normal_a", predicted_plane_normal_a[0])
        print("predicted_plane_normal_b", predicted_plane_normal_b[0])
        print("predicted_plane_normal_c", predicted_plane_normal_c[0])
        
        print("predicted_x_bottom_mid", predicted_x_bottom_mid[0])
        print("predicted_y_bottom_mid", predicted_y_bottom_mid[0])
        
        print("bounding_box_with_class[:, 1].unsqueeze(-1)", bounding_box_with_class[:, 1].unsqueeze(-1)[0])
        print("bounding_box_with_class[:, 2].unsqueeze(-1)", bounding_box_with_class[:, 2].unsqueeze(-1)[0])
        ...
        print("predicted_distance_raytracing", predicted_distance_raytracing[0])
        ...
        print("predicted_distance_raytracing_classical", predicted_distance_raytracing_classical[0])
        # Finish just for debugging
