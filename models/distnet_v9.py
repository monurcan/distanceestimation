...
class DistNet_v9(pl.LightningModule):
    def __init__(self, pretrained=True, learning_rate=1e-3):
        super(DistNet_v9, self).__init__()
    ...
    def forward(self, roi, bounding_box_with_class):
        ...
        predicted_height_deviation_coefficient, coefficient_height_priors = torch.chunk(
            self.fc_height(combined_features_height_priors), 2, dim=1
        )

        predicted_physical_height = (
            torch.exp(predicted_height_deviation_coefficient - 0.6)
            * height_prior_for_this_class
        )
        ...
        predicted_distance_height_priors = (
            predicted_distance_height_priors * z_to_distance_coefficient
        )
