...
# Use dataloader_v6
# for idathermal
class DistNet_v25(pl.LightningModule):
    def __init__(self, learning_rate=1e-3):
        super(DistNet_v25, self).__init__()
        ...
        # IMAGE_SIZE_X = 640
        # IMAGE_SIZE_Y = 512
        # CAMERA_FOV = 71.9092569 # horizontal
        # CAMERA_HEIGHT = 1.15
        # FOCAL LENGTH = 441.176

        # Camera parameters
        self.focal_length = nn.Parameter(torch.tensor(441.176), requires_grad=False)
        self.c_x = nn.Parameter(torch.tensor(640 / 2), requires_grad=False)
        self.c_y = nn.Parameter(torch.tensor(512 / 2), requires_grad=False)
        ...
        self.camera_height_above_water = 1.15
        self.plane_normal_a = 0.0
        self.plane_normal_b = -1.0
        self.plane_normal_c = 0.0

        fov_x = 71.9092569 * np.pi / 180.0  # radians
        ...
        self.fc_combined = nn.Sequential(
            nn.Linear(
                self.bbox_stream_output_size + self.bbox_input_size,
                256,
                bias=False,
            ),
            nn.LayerNorm(256),
        ...
        self.fc_height = nn.Sequential(
            nn.Linear(
                self.bbox_input_size + 1,  # for height priors[class id]
                512,
                bias=False,
            ),
            nn.LayerNorm(512),
        ...
        self.fc_raytracing = nn.Sequential(
            nn.Linear(self.bbox_input_size + 1, 512, bias=False),
            nn.LayerNorm(512),
            nn.GELU(),
            # nn.Dropout(0.2),
            nn.Linear(512, 256, bias=False),
    ...
    def forward(self, bounding_box_with_class):
        ...
        combined_features_blackbox = torch.cat(
            [processed_bbox, bounding_box_with_class],
            dim=1,
        )
    ...
    def training_step(self, batch, batch_idx):
        bounding_box_with_class, target_distance = batch
        (
            predicted_distance,
            predicted_distance_blackbox,
            predicted_distance_height_priors,
