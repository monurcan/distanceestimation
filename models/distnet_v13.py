...
class DistNet_v13(pl.LightningModule):
    def __init__(self, pretrained=True, learning_rate=1e-3):
        super(DistNet_v13, self).__init__()
    ...
    def forward(self, roi, bounding_box_with_class):
        ...
        # TODO: Try not to use small angle approximation in the next version.
        ...
        predicted_small_roll = 0.1 * predicted_small_roll  # radians
        predicted_small_pitch = 0.25 * predicted_small_pitch  # radians
        predicted_small_heave = (
            0.2 * predicted_small_heave + 1
        )  # meters - original heave * [0.8, 1.2]
        predicted_x_mid_deviation = predicted_x_mid_deviation * 15.0
        predicted_y_mid_deviation = predicted_y_mid_deviation * 15.0

        predicted_camera_height_above_water = (
            self.camera_height_above_water * predicted_small_heave
        )
        predicted_plane_normal_a = self.plane_normal_a + predicted_small_roll
        predicted_plane_normal_b = self.plane_normal_b
        predicted_plane_normal_c = self.plane_normal_c - predicted_small_pitch

        predicted_x_bottom_mid = (
            bounding_box_with_class[:, 1].unsqueeze(-1) + predicted_x_mid_deviation
        )
        predicted_y_bottom_mid = (
            bounding_box_with_class[:, 2].unsqueeze(-1) + predicted_y_mid_deviation
        )
