...
class DistNet_v15(pl.LightningModule):
    def __init__(self, pretrained=True, learning_rate=1e-3):
        super(DistNet_v15, self).__init__()
    ...
    def forward(self, roi, bounding_box_with_class):
        ...
        predicted_small_heave = (
            0.2 * predicted_small_heave + 1.0
        )  # meters - original heave * [0.8, 1.2]
        predicted_x_mid_deviation = predicted_x_mid_deviation * 0.6
        predicted_y_mid_deviation = predicted_y_mid_deviation * 0.6
        ...
        predicted_plane_normal_a = self.plane_normal_b*(-torch.sin(predicted_small_roll)*torch.cos(predicted_small_pitch))
        predicted_plane_normal_b = self.plane_normal_b*(torch.cos(predicted_small_roll)*torch.cos(predicted_small_pitch))
        predicted_plane_normal_c = self.plane_normal_b*torch.sin(predicted_small_pitch)
