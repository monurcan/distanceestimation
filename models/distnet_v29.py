...
# for dearsan
class DistNet_v29(pl.LightningModule):
    def __init__(self, learning_rate=1e-3):
        super(DistNet_v29, self).__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        final_loss = nn.MSELoss(reduction='none')(predicted_distance, target_distance) * 500.0 / (target_distance + 1e-6)
        final_loss = torch.mean(final_loss)
        black_box_loss = nn.MSELoss(reduction='none')(predicted_distance_blackbox, target_distance) * 500.0 / (target_distance + 1e-6)
        ...
        ) * 500.0 / (target_distance + 1e-6)
        height_prior_loss = torch.mean(height_prior_loss)
        raytracing_loss = nn.MSELoss(reduction='none')(predicted_distance_raytracing, target_distance) * 500.0 / (target_distance + 1e-6)
