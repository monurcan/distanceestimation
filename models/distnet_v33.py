...
from utils.losses import silog_loss

# silogloss denicem
class DistNet_v33(pl.LightningModule):
    def __init__(self, learning_rate=1e-3):
        super(DistNet_v33, self).__init__()
        ...
        self.camera_height_above_water = 3.56
        ...
        max_z = 1000
        self.max_distance = (
            np.sqrt(1 + (self.image_width / 2 * self.z_to_distance_coefficient) ** 2)
            * max_z
        )
        ...
        self.height_priors = nn.Parameter(
            torch.tensor(
                [
                    3.20,  # 0: ida - Internet: albatros-t 3.8, albatros-s 2.5
                    3.75,  # 1: boat - CARLA
                    12.4,  # 2: ship
                    0.2,  # 3: floatsam
        ...
        # print(dir(self.roi_backbone))
        # print(self.roi_backbone)
    ...
    def forward(self, roi, bounding_box_with_class):
        ...
        predicted_distance_blackbox = predicted_distance_blackbox * self.max_distance
        ...
        predicted_distance_raytracing_classical = torch.min(
            self.max_distance * for_clipping_mask,
            torch.max(
                predicted_distance_classical_raytracing_before_clipping,
    ...
    def training_step(self, batch, batch_idx):
        ...
        final_loss = silog_loss(predicted_distance, target_distance)
        black_box_loss = silog_loss(predicted_distance_blackbox, target_distance)
        height_prior_loss = silog_loss(
            predicted_distance_height_priors, target_distance
        )
        raytracing_loss = silog_loss(predicted_distance_raytracing, target_distance)
        ...
        self.log("train_loss_blackbox", black_box_loss, sync_dist=True)
        self.log("train_loss_height_prior", height_prior_loss, sync_dist=True)
        self.log("train_loss_raytracing", raytracing_loss, sync_dist=True)
        self.log("train_loss_smoothing", smoothing_loss, sync_dist=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_loss_l1", val_loss_l1, sync_dist=True)
        black_box_loss = nn.MSELoss()(predicted_distance_blackbox, target_distance)
        self.log("val_loss_blackbox", black_box_loss, sync_dist=True)
        black_box_loss = nn.L1Loss()(predicted_distance_blackbox, target_distance)
        self.log("val_loss_blackbox_l1", black_box_loss, sync_dist=True)
        height_prior_loss = nn.MSELoss()(
            predicted_distance_height_priors, target_distance
        )
        self.log("val_loss_heightprior", height_prior_loss, sync_dist=True)
        height_prior_loss = nn.L1Loss()(
            predicted_distance_height_priors, target_distance
        )
        self.log("val_loss_heightprior_l1", height_prior_loss, sync_dist=True)
        raytracing_loss = nn.MSELoss()(predicted_distance_raytracing, target_distance)
        self.log("val_loss_raytracing", raytracing_loss, sync_dist=True)
        raytracing_loss = nn.L1Loss()(predicted_distance_raytracing, target_distance)
        self.log("val_loss_raytracing_l1", raytracing_loss, sync_dist=True)

        classical_raytracing_loss = nn.L1Loss()(
            predicted_distance_raytracing_classical, target_distance
        )
        self.log(
            "val_loss_classical_raytracing_l1",
            classical_raytracing_loss,
            sync_dist=True,
        )
