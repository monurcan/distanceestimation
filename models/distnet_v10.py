...
class DistNet_v10(pl.LightningModule):
    def __init__(self, pretrained=True, learning_rate=1e-3):
        super(DistNet_v10, self).__init__()
    ...
    def forward(self, roi, bounding_box_with_class):
        ...
        (
            ...
            predicted_height_deviation_coefficient_sign,
            coefficient_height_priors,
        ) = torch.chunk(self.fc_height(combined_features_height_priors), 3, dim=1)

        predicted_height_deviation_coefficient = (
            1.0
            + 0.5
            * predicted_height_deviation_coefficient
            * torch.sign(predicted_height_deviation_coefficient_sign)
