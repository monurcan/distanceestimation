...
# v24 + alperen's suggestions
# for dearsan
class DistNet_v28(pl.LightningModule):
    def __init__(self, learning_rate=1e-3):
        super(DistNet_v28, self).__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        final_loss = nn.MSELoss(reduction='none')(predicted_distance, target_distance) * 200.0 / (target_distance + 1e-6)
        final_loss = torch.mean(final_loss)
        black_box_loss = nn.MSELoss(reduction='none')(predicted_distance_blackbox, target_distance) * 200.0 / (target_distance + 1e-6)
        black_box_loss = torch.mean(black_box_loss)
        height_prior_loss = nn.MSELoss(reduction='none')(
            predicted_distance_height_priors, target_distance
        ) * 200.0 / (target_distance + 1e-6)
        height_prior_loss = torch.mean(height_prior_loss)
        raytracing_loss = nn.MSELoss(reduction='none')(predicted_distance_raytracing, target_distance) * 200.0 / (target_distance + 1e-6)
        raytracing_loss = torch.mean(raytracing_loss)
