...
class YOLOLikeDataset(Dataset):
    ...
    def generate_random_scale():
        random_var1 = random.triangular(-0.1, +0.15, 0)
    ...
    def __getitem__(self, idx):
        ...
        for selected_line_i in range(len(valid_annotations)):
            ...
            # )
            width_deviation = self.generate_random_scale() * width
            height_deviation = self.generate_random_scale() * height
            x_mid_deviation = (self.generate_random_scale() * width - 1)/2 + 1
            y_mid_deviation = (self.generate_random_scale() * height - 1)/2 + 1
