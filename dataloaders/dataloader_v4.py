...
class YOLOLikeDataset(Dataset):
    def __init__(self, image_dir, label_dir, distance_dir, feature_dir):
    ...
    def __getitem__(self, idx):
        ...
        if len(valid_annotations) != len(valid_distances) or len(valid_features) != len(
            valid_annotations
        ):
            # Handle the case where the number of bounding boxes and distances are not equal
            return None

        valid_lines = list(zip(valid_annotations, valid_distances, valid_features))
        ...
        for selected_line_i in range(len(valid_annotations)):
            ...
            selected_line, selected_distance, selected_feature = valid_lines[
                selected_line_i
            ]
            ...
            # x_min = max(0, x_min - 2)
            # y_min = max(0, y_min - 2)
            # x_max = min(image_shape[1], x_max + 2)
            # y_max = min(image_shape[0], y_max + 2)
...
class DepthDataModule(pytorch_lightning.LightningDataModule):
    def __init__(self, image_dir, label_dir, distance_dir, feature_dir) -> None:
        super().__init__()
        self.image_dir = image_dir
        self.label_dir = label_dir
        self.distance_dir = distance_dir
